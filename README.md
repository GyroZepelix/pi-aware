...
- A `/usr/bin/say` failure warns once and disables speech until `/reload` or session replacement.
...
2. Inside tmux, trigger a blocking extension UI and listen for `question <window-index>`.
3. Finish and abort separate agent runs and listen for `finished <window-index>` after each settles.
4. Change or renumber the window index and confirm the next alert uses the new number.
5. Run outside tmux and confirm the same phrases omit the number.
6. Add a valid config, run `/reload`, and confirm the custom voice, rate, and phrases.
7. Add invalid config, run `/reload`, and confirm one warning followed by default behavior.
8. Where safe, induce a `say` failure, confirm one warning and no later speech, then repair the config and run `/reload`.
